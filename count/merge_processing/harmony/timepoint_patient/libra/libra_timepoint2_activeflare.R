...
seur <- readRDS("subset_2_2Tri.rds")
...
seur$cr.active.flare[seur$cr.patient %in% c('9', '11', '22', '23')] <- 'noflare'
seur$cr.active.flare[seur$cr.patient %in% c('4', '7', '12', '18')] <- 'noflare'
...
seur$cr.active.flare[seur$cr.patient == '14' & seur$cr.timepoint %in% c("1_1Tri", "3_3Tri")] <- 'noflare'
seur$cr.active.flare[seur$cr.patient == '14' & seur$cr.timepoint %in% c("2_2Tri")] <- 'flare'
seur$cr.active.flare[seur$cr.patient == '3' & seur$cr.timepoint %in% c("3_3Tri", '5_Post6M')] <- 'noflare'
seur$cr.active.flare[seur$cr.patient == '3' & seur$cr.timepoint %in% c("1_1Tri", "2_2Tri")] <- 'flare'
seur$cr.active.flare[seur$cr.patient == '20' & seur$cr.timepoint %in% c("3_3Tri", '5_Post6M')] <- 'noflare'
...
cell_type <- "bulk"
comparisons <- c("control-flare", "control-noflare", "noflare-flare")
