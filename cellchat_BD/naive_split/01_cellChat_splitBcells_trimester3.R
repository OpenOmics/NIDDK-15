workdir <- "/data/NHLBI_IDSS/rawdata/NIDDK-15_temp/cellchat_BD/naive_split/trimester3"
...
seur <- readRDS("/data/NHLBI_IDSS/rawdata/NIDDK-15_temp/cellchat_BD/trimester3/trimester3_subset.rds")

# ---- Rename B cells
b_cells <- read.csv("/data/NHLBI_IDSS/rawdata/NIDDK-15_temp/count/merge_processing/seurat_merge/B_cells/Bcells_3Tri_FCRL1_Naive_metadata.csv", row.names = 1)

seur <- AddMetaData(seur, b_cells[,c('RNA_snn_res.0.4', 'FCRL1_Naive')])
seur@meta.data <- seur@meta.data %>%
  mutate(predicted.celltype.l1.split_naive = coalesce(FCRL1_Naive, predicted.celltype.l1))

saveRDS(seur, 'trimester3_subset.rds')
...
create.cellchat.obj <- function(seurat_obj, CellChatDB){
  cellchat <- createCellChat(object = seurat_obj, group.by = "predicted.celltype.l1.split_naive", assay = "RNA")
...
saveRDS(cellchat.a, file = "trimester3_after_cellchat.rds")
...
saveRDS(cellchat.b, file = "trimester3_before_cellchat.rds")
...
saveRDS(cellchat.c, file = "trimester3_control_cellchat.rds")
...
saveRDS(cellchat.w, file = "trimester3_without_cellchat.rds")
...
saveRDS(cellchat.all, file = "trimester3_all_cellchat.rds")

capture.output(sessionInfo(), "trimester3_cellchat_sessionInfo.txt")
