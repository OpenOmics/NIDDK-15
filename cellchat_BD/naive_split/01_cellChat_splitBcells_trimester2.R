workdir <- "/data/NHLBI_IDSS/rawdata/NIDDK-15_temp/cellchat_BD/naive_split/trimester2"
...
seur <- readRDS("/data/NHLBI_IDSS/rawdata/NIDDK-15_temp/cellchat_BD/trimester2/trimester2_subset.rds")

# ---- Rename B cells
b_cells <- read.csv("/data/NHLBI_IDSS/rawdata/NIDDK-15_temp/count/merge_processing/seurat_merge/B_cells/Bcells_2Tri_FCRL1_Naive_metadata.csv", row.names = 1)

seur <- AddMetaData(seur, b_cells[,c('RNA_snn_res.0.2', 'FCRL1_Naive')])
seur@meta.data <- seur@meta.data %>%
  mutate(predicted.celltype.l1.split_naive = coalesce(FCRL1_Naive, predicted.celltype.l1))

saveRDS(seur, 'trimester2_subset.rds')
