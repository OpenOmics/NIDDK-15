...
subtitle: "Trimester `r params$timepoint`: `r params$subtitle`B Cells Significant Interactions Report"
author: "Vicky Chen"
date: "`r Sys.Date()`"
output:
    rmdformats::robobook:
...
for (i in names(cellchat@idents)) {
 cellchat@idents[[i]] <- relevel(cellchat@idents[[i]], 'FCRL1+ B cells')
}
```

## Significant Interaction Pairs {.tabset .tabset-fade}

The significant interactions for all flare groups with all B cell categories as a source and non B cell categories as a target was visualized.

```{r}
sources <- grep('B cell', unique(cellchat@idents$joint), value=T)
width <- (length(sources)*length(cellchat@netP)/3.5) + 2

```

```{r significant communications per celltype, results='asis', fig.width=width}

for (target in params$target %>% sort()) {
  cat ("### ", target, "{-}\n\n")
  print(netVisual_bubble(cellchat, sources.use = sources, targets.use=target, comparison=c(1, 2, 3, 4), angle.x = 45, remove.isolate = T))
  cat('\n\n')
}

width <- (length(sources)*length(cellchat@netP)*length(params$target))/4 + 2

```

```{r, significant communications all celltypes, results='asis', fig.height=9, fig.width=width}

cat ("### All Cell Types{-}\n\n")
print(netVisual_bubble(cellchat, sources.use = sources, targets.use=params$target, comparison=c(1, 2, 3, 4), angle.x = 45, remove.isolate = T))
cat("\n\n")
