...
subtitle: "Trimester 1: Without vs Uninfected"
author: "Brittany Dulek and Vicky Chen"
date: "`r Sys.Date()`"
output:
    html_document:
        toc: true
...
comparison <- c(3, 4) # 1 = after, 2 = before, 3 = uninfected, 4 = without 
```
...
# CellChat: Without vs Uninfected
CellChat differential analysis compared <font color=#337AB7>**trimester 1**</font> HBV-infected subjects without a flare and uninfected subjects.

## Interaction Counts and Weights {.tabset .tabset-fade}
Plots in this section will have the term differential indicating a subtraction between groups. The color <span style="color: red;">red</span> in the plots indicates <span style="color: red;">increased</span> signaling in the <span style="color: red;">without</span> group compared to the uninfected group and the color <span style="color: blue;">blue</span> indicates <span style="color: blue;">decreased</span> signaling in the <span style="color: blue;">without</span> group compared to the uninfected group.
...
The *Full Comparison tab* shows outgoing and incoming interaction strength comparison for <font color=#337AB7>**trimester 1**</font> uninfected (left) and without (right) flare groupings for each cell type.

The cell type specific tabs show the differential signaling changes between uninfected and without.  The plot is shown as a coordinate plane with 4 quadrants as shown in the image below.  Points in quadrant 1 will indicate increased incoming and outgoing signaling strength in the without group. Points in quadrant 3 will indicate increased incoming and outgoing signaling strength in the uninfected group.  

```{r quadrants, fig.align="center"}
include_graphics("/data/NHLBI_IDSS/rawdata/NIDDK-15_temp/cellchat_BD/quadrants.jpg")
```

The point colors indicate if a signal was seen in both the uninfected and without group, the uninfected group only, or the without group only.  The shapes indicate whether the signal was seen in incoming and outgoing for both the uninfected and without group (shared), incoming specific (for uninfected or without, color will indicate which), outgoing specific (for uninfected or without, color will indicate which), and incoming & outgoing specific (e.g. incoming in the uninfected group only and outgoing in the without group only).
...
For example if you look at the FCRL1+ B cells plot at the top of the plot in quadrant 1 will be a blue triangle labeled APP.  The location of APP in quadrant 1 of the coordinate plane of the plot indicates that incoming and outgoing signaling strength is higher in the without group.  The triangle indicates outgoing specific and the color indicates without specific.  This means that the outgoing signal was seen only in the without group.  This also means the incoming signal was seen in both the without and before groups.  Points other than black circles indicate that a signal was not present/detected in both groups.
