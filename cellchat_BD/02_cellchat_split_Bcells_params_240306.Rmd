...
# CellChat: `r params$source` to `r params$target`

## Significant Interaction Pairs {.tabset .tabset-fade}

The significant interactions with `r params$source` as a source and `r params$target` as a target was compared between the without group and each of the other flare groups to determine which communications networks have increased signaling in each group.

```{r Comparing communications, results='asis'}
source = which(cellchat.after@idents %>% levels == params$source)
target = which(cellchat.after@idents %>% levels == params$target)
...
The following chord plots are used to visualize all the significant signaling pathways from `r params$source` to `r params$target`. The color of the circles denotes the associated cell type. The thinner inner circle marks where the signal is targetting. Each segment of the circle represents a signaling pathway.

```{r chord gene, results='asis'}
plotChordGene <- function(group, cellchat) {
  cat("\n\n")
  cat("####", group, "{-}\n\n")
  print(netVisual_chord_gene(cellchat, sources.use=source, targets.use=target, slot.name="netP", lab.cex = 0.5, title.name = paste0("Signaling from ", params$source, " to ", params$target, " - ", group)))
  cat("\n\n")
}
...
pathways.show <- communication %>% filter(source == params$source & target == params$target) %>% pull(pathway_name) %>% unique %>% sort
```
...
The significant signaling pathways from `r params$source` to `r params$target` for each group. The tables below are returning interactions that have a p-value less than 0.05.

```{r communication table, results='asis'}
getCommnication <- function(group, cellchat) {
  df.net <- subsetCommunication(cellchat, sources.use=params$source, targets.use=params$target)
